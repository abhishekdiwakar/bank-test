...
/**
 * Functional test that demonstrate the key functionality of the banking service
 * and verifies that the solution works.
 * <p/>
 * Please ensure that this test pass without any failures before submitting your
 * solution.
 *
 * @author Unibet
 */
...
public final class BankFunctionalTest {
	...
	public void setupInitialDataBeforeEachTest() throws Exception {
		BankFactory bankFactory = (BankFactory) Class.forName("com.unibet.worktest.bank.BankFactoryImpl").newInstance();
	...
	public void accountBalancesUpdatedAfterTransfer() {
		...
		transferService.transferFunds(
				TransferRequest.builder().reference("tx1").account(CASH_ACCOUNT_1).amount(Money.euros("-5.00")).account(DEPOSIT_ACCOUNT_1).amount(Money.euros("5.00")).build());

		transferService.transferFunds(
				TransferRequest.builder().reference("tx2").account(CASH_ACCOUNT_1).amount(Money.euros("-10.50")).account(DEPOSIT_ACCOUNT_1).amount(Money.euros("10.50")).build());
	...
	public void accountBalancesUpdatedAfterMultiLegTransfer() {
		...
		transferService.transferFunds(TransferRequest.builder().reference("tx1").account(CASH_ACCOUNT_1).amount(Money.euros("-5.00")).account(DEPOSIT_ACCOUNT_1)
				.amount(Money.euros("5.00")).account(CASH_ACCOUNT_1).amount(Money.euros("-10.50")).account(DEPOSIT_ACCOUNT_1).amount(Money.euros("10.50")).account(CASH_ACCOUNT_1)
				.amount(Money.euros("-2.00")).account(DEPOSIT_ACCOUNT_1).amount(Money.euros("1.00")).account(DEPOSIT_ACCOUNT_1).amount(Money.euros("1.00")).build());
	...
	public void accountBalancesUpdatedAfterMultiLegMultiCurrencyTransfer() {
		...
		transferService.transferFunds(TransferRequest.builder().reference("tx1").account(CASH_ACCOUNT_1).amount(Money.euros("-5.00")).account(DEPOSIT_ACCOUNT_1)
				.amount(Money.euros("5.00")).account(CASH_ACCOUNT_2).amount(Money.kronor("-10.50")).account(DEPOSIT_ACCOUNT_2).amount(Money.kronor("10.50")).build());
